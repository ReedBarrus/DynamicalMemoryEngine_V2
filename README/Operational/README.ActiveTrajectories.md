...
| `TRJ-APP-001` | `App surface remodeling` | Move toward a thin router/home shell above dedicated live, static, and inspection viewers while keeping structural evidence primary and overlays subordinate. | `Leg 1 primary; depends into Legs 2 and 3` | `active` | `PKT-REORG-CLOSEOUT-AUDIT-007`; `PKT-ACTIVE-TRAJECTORIES-008`; `D1.UI.HOME_ROUTER_SHELL_013`; `D1.UI.SHARED_VIEWER_PAYLOAD_ADAPTER_014`; `D1.UI.MODE_ROUTER_EMPTY_VIEWERS_015`; `D1.UI.MODE_SHELL_STATE_THREADING_016`; `D1.UI.LIVE_TELEMETRY_RAIL_017` | `D1.UI.LIVE_TELEMETRY_RAIL_017` | `dedicated continuous/live viewer packet over bounded telemetry rail and real threaded state` | `README/Diagnostics/AppSurface/README.AppSurfaceTransitionPlan.v0.md`; `README/Diagnostics/AppSurface/README.StructuralViewerContract.v0.md`; `README/Diagnostics/AppSurface/README.LiveStaticInspectionSplit.v0.md`; `README/Diagnostics/AppSurface/README.LiveViewerTelemetryContract.v0.md`; current HUD/app surfaces; published shell-state seam in hud/shellStateRouter.js | richer viewer growth still needs bounded packets; export-time posture is visible but most cadence/jitter metrics remain unwired; legacy composed route should stay transitional until dedicated viewers are stronger | no roadmap rewrite; no styling-first rewrite; no overlay authority uplift; no per-viewer truth drift; no semantic interpretation from telemetry | park when the thin router, shared payload seam, explicit mode shells, current state-threading posture, and bounded live telemetry rail are stable enough that later packets are mostly incremental viewer hardening |
