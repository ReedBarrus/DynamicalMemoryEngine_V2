...
| `tests/hud/app_surfaces/` | `tests/hud/app_surfaces/` | verification_subgroup | active | door_one | HUD app-surface routing, mode-shell, state-threading, live-telemetry, and intake verification | `README.RepoPlacementConstitution.md`; `README.WorkflowContract.md` | strong | High-confidence app-surface tests now grouped by verification function rather than future-door rhetoric; current coverage includes router-shell, explicit mode-shell split, published shell-state threading, and Live-only telemetry rail verification. |
